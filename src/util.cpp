...
const char * const BITCOIN_CONF_FILENAME = "caffeecoin.conf";
const char * const BITCOIN_PID_FILENAME = "caffeecoin.pid";
...
#else
    const char* pszModule = "caffeecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Caffeecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Caffeecoin";
#else
    // Unix
    return pathRet / ".caffeecoin";
#endif
#endif
}
